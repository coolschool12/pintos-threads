...
void sys_exit(int status)
{
    ...
    while(!list_empty(&current->thread_files)) {
      struct file_descriptor * des = list_entry(list_begin(&current->thread_files), struct file_descriptor, elem);
      file_close(des->file_itself);
